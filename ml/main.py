...
from data_processing.model_training import model_trainer
...
def main():
    ...
    if raw_sd is not None and not raw_sd.empty:
        ...
        if (choice == 1):
            ...
            if(merged_sd is None or merged_sd is merged_sd.empty):
                ...
                if any([df is None or df.empty for df in [X_train, X_test, y_train, y_test]]):
                    ...
                    pipeline_path = Path(config["artifcats"]["pipelines"])
            ...
            model = model_trainer(sX_train_resampled, sy_train_resampled, sX_test, sy_test,"single")
