...
import PredictionImpactTable from "./PredictionImpaceTable";
import PredictionResult from "@/app/(main)/services/stroke-prediction/result/page";
  ...
  return (
    <motion.div
      ...
      className={`w-[350px] md:w-full md:max-w-3xl  rounded-3xl  flex justify-between items-center
            bg-conic/[from_var(--border-angle)] my-5  p-1 animate-rotate-border ${RiskColorCard(
      ...
      <Card className="w-screen rounded-3xl">
        <CardContent className="w-full grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="px-2 flex :justify-start items-center md:items-start flex-col">
            <h3
              className={`text-xl font-bold w-fit px-2 tracking-wide text-muted-foreground`}
            >
              Risk Level
            </h3>
            <p
              className={`text-lg font-medium capitalize text-center w-fit mt-2 ml-2  px-2 border-2 rounded-xl tracking-wider shadow-md
                  ${RiskColorTag(predictionResult.risk_level)}`}
            >
              {predictionResult.risk_level}
            </p>
          </div>
          <div className="px-2 mx-auto">
            <h3 className="text-xl font-bold w-fit px-2 tracking-wide text-muted-foreground">
              Stroke Chance
            </h3>
          ...
          {predictionResult.factors.length > 0 && (
            <div className="col-span-full px-2">
              ...
              <PredictionImpactTable factors={predictionResult.factors}/>
            </div>
          )}
        </CardContent>
      </Card>
